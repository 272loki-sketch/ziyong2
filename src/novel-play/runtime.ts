...
export interface NovelPlayCardBinding {
	docId: string;
	revision: string;
	startNodeId: string;
...
export interface NovelPlayRuntimeState {
	version: 1;
	card: NovelPlayCardBinding;
	progress: NovelAnchor;
	conflicts: NovelConflict[];
	preparedFromLeafId: string;
}

export interface NovelPlayProjection {
	version: 1;
	packageRevision: string;
	anchor: NovelAnchor;
	candidates: Array<{ nodeId: string; title: string; summary: string; actuality: "candidate" }>;
	blockedNodeIds: string[];
	omittedNodeIds: string[];
	usedChars: number;
}

export interface PreparedNovelPlayTurn {
	projection: NovelPlayProjection;
	state: NovelPlayRuntimeState;
}
...
export interface PrepareNovelPlayTurnInput {
	...
	loadPackage?: typeof loadNovelPackage;
}
...
export function novelPlayCardBinding(rawCard: unknown): NovelPlayCardBinding | undefined {
	const root = record(rawCard);
	const data = record(root?.data) ?? root;
	const extension = record(record(data?.extensions)?.liyuanNovelPlay);
	if (!extension) return undefined;
	const docId = text(extension.docId);
	const revision = text(extension.revision);
	const startNodeId = text(extension.startNodeId);
...
function sameCard(a: NovelPlayCardBinding, b: NovelPlayCardBinding): boolean {
	return a.docId === b.docId && a.revision === b.revision && a.startNodeId === b.startNodeId && a.position === b.position;
}

function validAnchor(pkg: NovelPackage, value: unknown): NovelAnchor | undefined {
	const raw = record(value);
	const nodeId = text(raw?.nodeId);
	const packageRevision = text(raw?.packageRevision);
	const position = raw?.position;
	if (packageRevision !== pkg.revision || !pkg.nodes.some(node => node.id === nodeId)
		|| (position !== "before" && position !== "after")) return undefined;
...
function conflictsOf(pkg: NovelPackage, value: unknown, ancestorIds: ReadonlySet<string>): NovelConflict[] | undefined {
	...
	for (const item of value) {
		...
		const nodeId = text(raw?.nodeId);
		if (text(raw?.packageRevision) !== pkg.revision || !pkg.nodes.some(node => node.id === nodeId)
			|| !Array.isArray(raw?.sourceEntryIds)) return undefined;
...
export function novelPlayStateFromBranch(branch: BranchEntryLike[], binding: NovelPlayCardBinding, pkg: NovelPackage): NovelPlayRuntimeState | undefined {
	...
	for (let index = branch.length - 1; index >= 0; index--) {
		...
		const raw = record(entry.data);
		const card = record(raw?.card) as NovelPlayCardBinding | undefined;
		const progress = validAnchor(pkg, raw?.progress);
		const conflicts = conflictsOf(pkg, raw?.conflicts, ids);
		const preparedFromLeafId = text(raw?.preparedFromLeafId);
		if (raw?.version === 1 && card && sameCard(card, binding) && progress && conflicts && preparedFromLeafId && ids.has(preparedFromLeafId)) {
			return { version: 1, card: binding, progress, conflicts, preparedFromLeafId };
		}
	}
	return undefined;
}
...
function branchFacts(branch: BranchEntryLike[]): Array<{ entryId: string; kind: string; text?: string; data?: unknown }> {
	...
	for (const entry of branch) {
		...
		const role = text(message?.role) || text(entry.type);
		...
		} else if (entry.type === "custom" && ["rp-state", "rp-world-state", "rp-ecology-state", "rp-summary"].includes(text(entry.customType))) {
			row = { entryId, kind: text(entry.customType), data: entry.data };
		} else if (entry.type === "custom_message" && ["rp-greeting", "rp-edited-reply", "rp-import"].includes(text(entry.customType))) {
			row = { entryId, kind: text(entry.customType), text: text(entry.content) };
		}
		if (!row) continue;
...
function parseModelResult(value: string): Record<string, unknown> | undefined {
	try { return record(JSON.parse(value)); } catch { return undefined; }
}

/**
 * Builds a read-only, per-turn projection from the current Session Tree branch.
 * Canon summaries are candidates, never facts. Full source chunks and card books are never model input.
 */
export async function prepareNovelPlayTurn(input: PrepareNovelPlayTurnInput): Promise<PreparedNovelPlayTurn | undefined> {
	...
	const stored = (input.loadPackage ?? loadNovelPackage)(input.cwd, binding.docId, binding.revision);
	...
	if (!validAnchor(pkg, startingAnchor)) return undefined;
	const currentNode = pkg.nodes.find(node => node.id === startingAnchor.nodeId)!;
	const canonical = pkg.nodes.filter(node => node.stageId === currentNode.stageId && node.visibility === "public")
		.map(node => ({ nodeId: node.id, order: node.order, title: node.title, summary: node.summary, dependsOn: node.dependsOn }));
	...
	const progress = validAnchor(pkg, parsed.progress);
	const conflicts = conflictsOf(pkg, parsed.conflicts, ids);
	if (!progress || !conflicts || progress.nodeId !== currentNode.id) return undefined;
	...
	return {
		projection: { version: 1, packageRevision: pkg.revision, anchor: progress, ...projected },
		state: { version: 1, card: binding, progress, conflicts, preparedFromLeafId: input.expectedLeafId },
	};
}

/** Persist the validated projection metadata only. It never stores canon text or a second fact set. */
export function commitNovelPlayState(input: {
	prepared: PreparedNovelPlayTurn | undefined;
	expectedLeafId: string | null;
	getLeafId: () => string | null;
