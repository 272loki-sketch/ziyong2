...
| `<NGINX_SERVER_FRAGMENT_PATH>` | 实际生效的 Nginx server 片段路径 |
| `<HTPASSWD_PATH>` | Basic Auth 凭据文件路径 |
| `<TLS_CERT_DIR>` | 证书目录 |
| `<CERT_VALID_FROM>` | 证书生效时间 |
| `<CERT_VALID_TO>` | 证书失效时间 |
| `<BACKUP_ROOT>` | 备份根目录 |
| `<TIMESTAMP>` | 备份时间戳 |
| `<AGENT_DATA_DIR_PARENT>` | Agent 数据目录的父目录 |
...
研究搜索返回 0 条时，先检查 `GET /api/outline/research/search/schedule` 和服务日志，不要把搜索引擎原始页面直接导入素材库。当前系统会拒绝低相关结果。DuckDuckGo 人机验证或 Bing 兜底异常时，空结果是保护行为。查看完整根因、实测结果和清理范围：[`docs/INCIDENT-20260901-RESEARCH-SEARCH.md`](../docs/INCIDENT-20260901-RESEARCH-SEARCH.md).
