...
- **故事导演（导演室）**：主输入框右侧「导演室」按钮打开独立工作台。支持五种讨论模式——综合编剧、下一拍、下一段对白、角色反应、节奏诊断——导演结合当前正文、人物状态、世界与生态工件给出即时建议（推荐下一拍、起手、角色动作、建议聊天对象、切入话题、风险、混合路线）；讨论以 `rp-outline-chat` 存分支，不进入正文。真正要固化的长期方向进入建议箱，只有你确认后才写入动态大纲 `rp-outline`。导演室与系统助手（耳麦）职责分离。详见 [导演室](docs/DIRECTOR-ROOM.md)；
