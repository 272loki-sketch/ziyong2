...
import { deleteStageSkill, sanitizeSkillDir, saveStageSkill, scanSkillFiles } from "../src/stage/skill-store.ts";
...
test("saveStageSkill：写出的 SKILL.md 正好是 scanSkillFiles 读的格式（编辑器产物=引擎消费物）", () => {
	...
	try {
		...
		assert.equal(dir, "打斗");
		assert.ok(existsSync(join(cwd, ".liyuan-stage-skills", "打斗", "SKILL.md")));
...
test("saveStageSkill：同名保存更新用户覆盖", () => {
	const cwd = mkcwd();
	try {
		saveStageSkill(cwd, { name: "静场", description: "x", resident: false, body: "y" });
		saveStageSkill(cwd, { name: "静场", description: "z", resident: false, body: "w" });
		assert.equal(scanSkillFiles(cwd)[0]?.description, "z");
	} finally {
		rmSync(cwd, { recursive: true, force: true });
	}
});
...
test("saveStageSkill：description 里的换行被压平（frontmatter 按行解析，换行会截断）", () => {
	...
	try {
		...
		const raw = readFileSync(join(cwd, ".liyuan-stage-skills", "多行", "SKILL.md"), "utf8");
...
test("deleteStageSkill：删整目录；不存在报错；非 skill 目录不误删", () => {
	...
	try {
		...
		assert.throws(() => deleteStageSkill(cwd, "删我"), /内置 skill 不能删除|已有覆盖/);
		mkdirSync(join(cwd, "skills", "非skill"), { recursive: true });
		writeFileSync(join(cwd, "skills", "非skill", "README.md"), "x");
		assert.throws(() => deleteStageSkill(cwd, "非skill"), /内置 skill 不能删除|已有覆盖/, "无 SKILL.md 的目录不认");
...
test("世界适配三阶段 Skill 均被 workflow 注册表识别", () => {
	const repo = new URL("..", import.meta.url).pathname;
	const workflows = new Set(scanSkillFiles(repo).map((skill) => skill.workflow));
	assert.equal(workflows.has("world-profile"), true);
	assert.equal(workflows.has("world"), true);
	assert.equal(workflows.has("world-facts"), true);
	assert.equal(workflows.has("world-audit"), true);
});
...
test("大纲工作流 Skill 均被 workflow 注册表识别且不常驻", () => {
	...
	for (const workflow of ["outline-chat", "outline-bootstrap", "outline-reconcile", "outline-foreshadowing", "outline-research", "outline-corpus-research", "outline-audit"] as const) {
		...
		assert.equal(skill.everyBeat, false);
	}
});
