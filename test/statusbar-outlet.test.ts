...
test("mergeFinalText：空稿时纯状态栏或 CSS 围栏不能成为正文", () => {
	assert.equal(mergeFinalText("", "<state1>地点：山门</state1>"), "");
	assert.equal(mergeFinalText("", "```css\n.status { color: red; }\n```"), "");
	assert.equal(mergeFinalText("", "池宽治是故事中的主要人物。"), "池宽治是故事中的主要人物。");
});
